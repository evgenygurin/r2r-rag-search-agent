...
- Поддержка нескольких транспортов (HTTP, stdio) через единый API
- server.py:105 — инициализация FastMCP сервера
- server.py:152 — экспорт ASGI приложения для production (`app = mcp.http_app()`)
...
```bash
# Через Makefile (HTTP на порту 8000)
make run

# Прямой запуск (HTTP на порту 8000)
python server.py
```

**Production деплой (ChatMCP, Uvicorn):**

Сервер экспортирует ASGI приложение через `app = mcp.http_app()` (server.py:152).
...
1. **`POST /mcp 400 Bad Request`** при деплое на ChatMCP/Uvicorn
   - **Причина:** Сервер не экспортирует ASGI приложение (`app`)
   - **Решение:** Убедись, что в server.py есть `app = mcp.http_app()` на уровне модуля
   - **Проверка:** В логах должен быть `INFO: Application startup complete`

2. **`RuntimeError: Type not yet supported: str | None`** при `mcp install`
...
3. **"R2R connection refused"** → проверь `R2R_BASE_URL` в `.env` или переменной окружения

4. **"MCP tool not appearing"** в Claude Desktop → перезапусти Claude Desktop (Cmd+Q, затем открой)

5. **Empty search results** → проверь, что R2R база проиндексирована и доступна

6. **Invalid -W option ignored: invalid action: '"ignore'**
