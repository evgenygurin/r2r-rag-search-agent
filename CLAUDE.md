...
- server.py:154 — экспорт ASGI приложения для production (`app = mcp.streamable_http_app(path="/mcp")`)
...
Сервер экспортирует ASGI приложение через `app = mcp.streamable_http_app(path="/mcp")` (server.py:154).
**Важно:**
- Используется Streamable HTTP transport - рекомендуемый для production деплоев
- `path="/mcp"` — явно указан endpoint путь, ожидаемый MCP клиентами (ChatMCP, Claude Desktop и др.)
...
2. **`POST /mcp 400 Bad Request`** при деплое на ChatMCP/Uvicorn
   ...
   - **Решение:** Убедись, что в server.py есть `app = mcp.streamable_http_app(path="/mcp")` на уровне модуля
