...
- server.py:153 — экспорт ASGI приложения для production (`app = mcp.streamable_http_app()`)
...
Сервер экспортирует ASGI приложение через `app = mcp.streamable_http_app()` (server.py:153).
**Важно:** Используется Streamable HTTP transport - рекомендуемый для production деплоев.
...
1. **`'FastMCP' object has no attribute 'http_app'`** при деплое
   - **Причина:** Метод `http_app()` может отсутствовать в некоторых версиях FastMCP
   - **Решение:** Используй `app = mcp.streamable_http_app()` (рекомендуется для production)
...
2. **`POST /mcp 400 Bad Request`** при деплое на ChatMCP/Uvicorn
   ...
   - **Решение:** Убедись, что в server.py есть `app = mcp.streamable_http_app()` на уровне модуля
